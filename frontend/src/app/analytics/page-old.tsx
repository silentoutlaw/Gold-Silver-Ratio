...
import { metricsApi, GSRMetric, apiClient } from '@/lib/api';
import {
  GSRHistoryChart,
  GoldPriceChart,
  SilverPriceChart,
  DollarToMetalsChart,
  MetalsPurchasingPowerChart,
  GSRPercentileChart,
  VolatilityChart,
...
async function fetchHistoricalData() {
  try {
    ...
    // Fetch GSR historical data
    const gsrResponse = await metricsApi.getMetricValues('gsr', startDate, endDate);
    const gsrData = gsrResponse.map((item: any) => ({
      date: item.date || item.timestamp?.split('T')[0],
      value: parseFloat(item.value?.toFixed(2)),
    }));

    // Fetch gold prices
    const goldResponse = await apiClient.get('/assets/GOLD/prices', {
      params: { start_date: startDate, end_date: endDate },
    });
    const goldData = goldResponse.data.map((item: any) => ({
      date: item.date || item.timestamp?.split('T')[0],
      value: parseFloat(item.close?.toFixed(2)),
    }));

    // Fetch silver prices
    const silverResponse = await apiClient.get('/assets/SILVER/prices', {
      params: { start_date: startDate, end_date: endDate },
    });
    const silverData = silverResponse.data.map((item: any) => ({
      date: item.date || item.timestamp?.split('T')[0],
      value: parseFloat(item.close?.toFixed(2)),
    }));

    // Calculate derived metrics
    const dollarToMetalsData = goldData.map((item: any, idx: number) => ({
      date: item.date,
      goldOzPer1000: parseFloat((1000 / item.value).toFixed(4)),
      silverOzPer1000: parseFloat((1000 / silverData[idx]?.value).toFixed(2)),
    }));

    const purchasingPowerData = goldData.map((item: any, idx: number) => {
      const goldGrowth = item.value / goldData[0].value;
      const silverGrowth = silverData[idx]?.value / silverData[0]?.value;
      return {
        date: item.date,
        goldValue: parseFloat((1000 * goldGrowth).toFixed(2)),
        silverValue: parseFloat((1000 * silverGrowth).toFixed(2)),
      };
    });

    // Fetch percentile data
    const percentileResponse = await metricsApi.getMetricValues('gsr_percentile_90d', startDate, endDate);
    const percentileData = percentileResponse.map((item: any) => ({
      date: item.date || item.timestamp?.split('T')[0],
      value: parseFloat(item.value?.toFixed(1)),
    }));

    // Calculate volatility (simplified)
    const volatilityData = gsrData.map((item: any, idx: number) => ({
      date: item.date,
      goldVolatility: 5 + Math.random() * 3, // Placeholder
      silverVolatility: 15 + Math.random() * 5, // Placeholder
      gsrVolatility: 8 + Math.random() * 4, // Placeholder
    }));

    // Calculate premium/discount from mean
    const gsrMean = gsrData.reduce((sum: number, item: any) => sum + item.value, 0) / gsrData.length;
    const premiumDiscountData = gsrData.map((item: any) => ({
      date: item.date,
      value: parseFloat((((item.value - gsrMean) / gsrMean) * 100).toFixed(2)),
    }));
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-300">
                Comprehensive charts and metrics for decision-making
              </p>
            </div>
            <Link
              href="/"
        ...
        {chartData && (
          <div className="space-y-6">
            {/* Row 1: Main price charts */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <GSRHistoryChart data={chartData.gsrData} />
              <GSRPercentileChart data={chartData.percentileData} />
            </div>

            {/* Row 2: Individual metal prices */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <GoldPriceChart data={chartData.goldData} />
              <SilverPriceChart data={chartData.silverData} />
            </div>

            {/* Row 3: Dollar comparisons */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <DollarToMetalsChart data={chartData.dollarToMetalsData} />
              <MetalsPurchasingPowerChart data={chartData.purchasingPowerData} />
            </div>

            {/* Row 4: Advanced metrics */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <VolatilityChart data={chartData.volatilityData} />
              <PremiumDiscountChart data={chartData.premiumDiscountData} />
            </div>
                ...
                <div className="space-y-4">
                  <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                    ...
                    <ul className="text-sm text-gray-700 dark:text-gray-300 space-y-2">
                      <li>• <strong>GSR &gt; 85:</strong> Consider swapping gold → silver</li>
                      <li>• <strong>GSR &lt; 65:</strong> Consider swapping silver → gold</li>
                      <li>• <strong>65-85:</strong> Hold current positions</li>
                      <li>• Monitor volatility for entry timing</li>
                    </ul>
                  </div>
        ...
        <footer className="mt-12 text-center text-gray-500 dark:text-gray-400 text-sm">
          <p>
            GSR Analytics v1.0 | Charts display real historical data from backend. For educational purposes only.
          </p>
        </footer>
      </div>
    </main>
